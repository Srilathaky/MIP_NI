...
public class MIP_ListUser_Test {
	...
	public void test_setup(@Optional("firefox") String browser, @Optional("0") String flag) {
		log = MIP_Logging.logDetails("MIP_List_User_Test");
		report = new ExtentReports(".\\Test_Reports\\List_User_Test.html");
		if (flag.equals("0")) {
			try {
	...
	public void listUserOne() throws Throwable {
		testcasename = "MIP_webPortal_TC48_List_User";
		try {
			logger = report.startTest("List User-TC48");
			log.info("Running test case - TC48");
			logger.log(LogStatus.INFO, "Validating List User Page Objects");
			MIP_ListUserPage listuserpage = new MIP_ListUserPage(MIP_Test_Configuration.driver);
			Assert.assertTrue(listuserpage.validateListUserHeader());
			Assert.assertTrue(listuserpage.validateUserData());
	...
	public void listUserTwo(String mobileNumber) throws Throwable {
		testcasename = "MIP_webPortal_TC50_List_User";
		try {
			logger = report.startTest("List User-TC50");
			log.info("Running test case - TC50");
			MIP_ListUserPage listuserpage = new MIP_ListUserPage(MIP_Test_Configuration.driver);
			listuserpage.clickOnUser(mobileNumber);
			Map<String, String> userData = MIP_UserManagement_Queries.getUserInfo(mobileNumber);
			logger.log(LogStatus.INFO, "Validating User Information in List User Page for msisdn " + mobileNumber);
			Assert.assertTrue(userData.get("user_uid").equalsIgnoreCase(listuserpage.getUserId().trim()));
			Assert.assertTrue((userData.get("fname") + " " + userData.get("sname"))
					.equalsIgnoreCase((listuserpage.getFirstName().trim() + " " + listuserpage.getLastName().trim())));
			if (userData.get("email_id") != null)
				Assert.assertTrue(userData.get("email_id").equalsIgnoreCase(listuserpage.getEmailId().trim()));
			Assert.assertTrue(userData.get("gender").equalsIgnoreCase(listuserpage.getGender().trim()));
			Assert.assertTrue(userData.get("role_name").equalsIgnoreCase(listuserpage.getRole().trim()));
			Assert.assertTrue(userData.get("branch_name").equalsIgnoreCase(listuserpage.getBranch().trim()));
			if (userData.get("dob") != null) {
				String[] date_data = MIP_DateFunctionality.getDate(listuserpage.getdob(), MIP_Constants.DOB_FORMAT);
			...
			Assert.assertTrue(MIP_Test_Configuration.driver.findElement(By.id("backBtn")).isDisplayed());
			MIP_Test_Configuration.driver.findElement(By.id("backBtn")).click();
			logger.log(LogStatus.PASS, "Test passed");
		} catch (Throwable t) {
	...
	@Test(testName = "TC51", dataProvider = "ListUserData", dataProviderClass = MIP_ListUser_TestData.class)
	public void listUserThree(String mobileNumber) throws Throwable {
		testcasename = "MIP_webPortal_TC51_List_User";
		try {
			logger = report.startTest("List User-TC51");
			log.info("Running test case - TC50");
			MIP_ListUserPage listuserpage = new MIP_ListUserPage(MIP_Test_Configuration.driver);
			listuserpage.clickOnUser(mobileNumber);
			logger.log(LogStatus.INFO, "Validating Back Buton Functionality in List User Page");
			listuserpage.clickOnBack();
			Assert.assertTrue(listuserpage.validateListUserHeader());
			Assert.assertTrue(listuserpage.validateUserData());
			logger.log(LogStatus.PASS, "Test passed");
		} catch (Throwable t) {
			log.info("Testcase TC51  Failed");
			log.info("Error occured in the test case", t);
			throw t;
		}
	}
