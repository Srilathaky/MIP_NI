...
public class MIP_LeaveManagement_TestData {
	...
	public static String[][] validateApplyLeaveTestData() {
		String[][] data = { { "Fatima Duriez", "20/04/2018", "22/04/2018", "Sick" } };
		return data;
	}
	...
	public static String[][] storeCellData() {
		...
		for (int i = 0; i < cellnum; i++) {
			if (s.getRow(0).getCell(i).getStringCellValue().equalsIgnoreCase("From Date")) {
				From_Date = i;

			}
			if (s.getRow(0).getCell(i).getStringCellValue().equalsIgnoreCase("To Date")) {
				To_Date = i;
	...
	public static String[][] viewLeaveTestData() {
		String[][] data = { { "01/03/2018", "30/03/2018" } };
		return data;
	}
}
