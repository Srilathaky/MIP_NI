...
public class MIP_AddUserPage extends MIP_BasePage {
	...
	public WebElement supervisoryes;
	@FindBy(xpath = "//input[@id='isSupervisor'][@value='0']")
	public WebElement supervisorno;
	@FindBy(id = "saveBtn")
	...
	public MIP_AddUserPage enterUserInfo(String fname, String sname,
			...
			String role, String branch, String supervisor) {
		...
		if (!supervisor.equals("")) {
			if (supervisor.equalsIgnoreCase("yes")) {
				supervisoryes.click();
			} else if (supervisor.equalsIgnoreCase("no")) {
				supervisorno.click();
			}
		}
		if (!dob.equals("")) {
