...
public class MIP_ListBranchPage extends MIP_BasePage {
	...
	public List<WebElement> validatelistBranchObjects() {
		this.waitForElementToPresent(By.id("branchList"));
		return driver.findElements(By.xpath("//table[@id='branchList']/thead/tr/th"));
	}
		...
		for (int i = 0; i <= (Math.floor(count / 10)); i++) {
			try {
				...
				this.waitForElementToVisible(By.xpath(
						"//table[@id='branchList']/tbody//tr//td/a[contains(text(),'" + branchname.trim() + "')]"));
				driver.findElement(By.xpath(
						"//table[@id='branchList']/tbody//tr//td/a[text()='" + branchname.trim() + "']"))
						.click();
	...
	public String getBranchCode(String branchname) {
		...
		for (int i = 0; i < count / 4; i++) {
			try {
				driver.findElement(
						By.xpath("//table[@id='branchList']/tbody//tr//td/a[contains(text(),'" + branchname + "')]"));
				text = driver.findElement(By.xpath("//table[@id='branchList']/tbody//tr//td/a[text()='" + branchname
						+ "']//parent::td//preceding-sibling::td")).getText();
				break;
			} catch (Exception e) {
				driver.findElement(By.linkText("Next")).click();
			}
	...
	public String getBranchInfo(String value) {
		if (value.equalsIgnoreCase("name")) {
			...
			return this.waitForElementToVisible(By.id("branchName")).getAttribute("value");
		} else if (value.equalsIgnoreCase("street")) {
			return this.waitForElementToPresent(By.id("branchStreet")).getAttribute("value");
		} else if (value.equalsIgnoreCase("region")) {
			return this.waitForElementToPresent(By.id("branchRegion")).getAttribute("value");
		} else if (value.equalsIgnoreCase("city")) {
			return this.waitForElementToPresent(By.id("branchCity")).getAttribute("value");
		} else
			return null;
	}

	public MIP_ListBranchPage editBranchInfo(String branchname, String street, String region, String city) {
		if (!branchname.equalsIgnoreCase("")) {
			WebElement ele = this.waitForElementToPresent(By.id("branchName"));
			ele.clear();
			ele.sendKeys(branchname);
		}
		if (!street.equalsIgnoreCase("")) {
			WebElement ele = this.waitForElementToPresent(By.id("branchStreet"));
			ele.clear();
			ele.sendKeys(street);
		}
		if (!region.equalsIgnoreCase("")) {
			WebElement ele = this.waitForElementToPresent(By.id("branchRegion"));
			ele.clear();
			ele.sendKeys(region);
		}
		if (!city.equalsIgnoreCase("")) {
			WebElement ele = this.waitForElementToPresent(By.id("branchCity"));
			ele.clear();
			ele.sendKeys(city);
		}
		return this;
	...
	public String validatelistbranch() {
		return this.waitForElementToVisible(By.xpath("//div[@class='pagetitle']")).getText();
